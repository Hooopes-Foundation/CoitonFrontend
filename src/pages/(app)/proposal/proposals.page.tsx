...
export default function ProposalsPage() {
  return (
    <div className="flex flex-col xl:flex-row gap-6">
      <div className="xl:sticky top-24 flex h-max xl:w-[329px] flex-col rounded-xl border border-[#BEDAD9] bg-[#F5FFFE] p-8">
        <p className="mb-1 font-medium text-xl md:text-2xl">Coiton DAO</p>
        <p className="mb-10 text-sm md:text-base text-muted-foreground">
          Create your DAOs proposals
        </p>

        <Link to="/proposals/new">
          <Button className="mt-auto w-full rounded-full" size={"lg"}>
            <svg
              className="size-6"
              viewBox="0 0 25 25"
              fill="none"
      ...
      <div className="flex flex-1 flex-col gap-6 pb-6">
        {proposals.map((proposal, _index) => (
          <Link
            ...
            className="flex-1 rounded-xl border border-[#BEDAD9] bg-[#F5FFFE] px-4 md:px-6 py-4"
          >
            <div className="flex flex-col border-l border-[inherit]">
              <div className="flex items-center justify-between border-b border-[inherit] px-4 md:px-6 pb-4 pt-3">
              ...
              <div className="px-4 md:px-6 pt-4">
                ...
                <p className="md:line-clamp-2 font-regular text-sm md:text-lg">
                ...
                {proposal.pole && (
                  <div className="mt-6 flex flex-col gap-4">
                    {proposal.pole.map((percent, _index) => (
                      <div
                        key={_index}
                        className="relative h-10 md:h-12 overflow-hidden rounded-[8px] border border-[#82BEB9] bg-[#D4EBE9]"
                      >
                        <span
                          className="absolute left-0 top-0 h-full lg:w-[90%] bg-primary bg-blend-difference"
                          style={{ width: `${percent.selected}%` }}
                        />
                        <div className="flex size-full items-center justify-between px-5">
                          <p className="relative font-medium text-white text-sm md:text-base">
                            {percent.label}
                          </p>
                          <p className="relative font-medium text-white text-sm md:text-base">
                            {percent.selected}%
                ...
                <p className="mt-6 font-regular text-base md:text-lg text-muted-foreground">
