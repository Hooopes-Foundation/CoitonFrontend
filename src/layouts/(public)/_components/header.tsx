          ...
          {nav_routes.map((route: ROUTES) => {
            ...
            return (
              <li key={route.path} className="group relative">
                {isActive ? (
                  <span className="cursor-pointer px-2 font-sans_medium text-lg capitalize leading-none text-primary">
                    {route.label}
                  </span>
                ) : (
                  <Link
                    to={route.path}
                    className="cursor-pointer px-2 font-sans_medium text-lg capitalize leading-none text-primary"
                  >
                    {route.label}
                  </Link>
                )}
            ...
            <SheetContent className="flex flex-col justify-between">
              <SheetHeader className="mt-12">
                {nav_routes.map((route: ROUTES) => (
                  ...
                  >
                    <p className="text-left font-sans_medium text-base capitalize text-primary md:text-lg">
                      {route.label}
                    </p>
                  </SheetClose>
                ))}
