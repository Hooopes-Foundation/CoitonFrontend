...
>(({ className, index, type = "document", children, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "relative flex h-8 cursor-pointer items-center justify-between px-2",
        className,
        isSelected ? "bg-muted" : "",
        type === "media" ? "aspect-[1.2] h-32 w-max" : "w-full",
      )}
      {...props}
    >
      ...
      <button
        ...
        className={cn(
          ...
          type === "media" ? "absolute right-3 top-2" : "",
        )}
        onClick={() => removeFileFromSet(index)}
      >
        <span className="sr-only">remove item {index}</span>
        <X className="h-4 w-4 duration-200 ease-in-out hover:stroke-destructive" />
      </button>
    </div>
  );
});
